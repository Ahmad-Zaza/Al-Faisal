...
class SEOController extends Controller
{
    ...
    {
        if(! CRUDBooster::myId()) return  redirect(config('crudbooster.ADMIN_PATH') .'/login');
    ...
    public function store($model, Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $seo->save();
            // if($model && $model != "contact-us" && $model != "portfolio")
            //     return CRUDBooster::redirect(CrudBooster::adminPath("$model"), trans("crudbooster.alert_update_data_success"), 'success');
            return CRUDBooster::redirectBack(trans("crudbooster.alert_update_data_success"), 'success');
        }
    }
}